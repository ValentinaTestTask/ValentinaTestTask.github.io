...
$arParams["IBLOCK_ID2"] = trim($arParams["IBLOCK_ID2"]);
...
if($this->startResultCache(false, array(($arParams["CACHE_GROUPS"]==="N"? false: $USER->GetGroups()), $bUSER_HAVE_ACCESS, $arNavigation, $arrFilter, $pagerParameters)))
{
	...
	$arFilter = array (
			...
			"IBLOCK_LID" => SITE_ID,
			"PROPERTY_ATT_VENDOR" => $arParams["ELEMENT_ID"],
			"ACTIVE" => "Y",
	...
	if($arParams["ELEMENT_ID"] <= 0)
		...
		$rsElement2 = CIBlockElement::GetList(array(), $arFilter2, false, false, $arSelect);
	$rsElement2->SetUrlTemplates($arParams["DETAIL_URL"], "", $arParams["IBLOCK_URL"]);
	if($obElement = $rsElement2->GetNextElement())
	{
		$ipropValues = new Iblock\InheritedProperty\ElementValues($arResult["IBLOCK_ID2"], $arResult["ID"]);
		$arResult["IPROPERTY_VALUES2"] = $ipropValues->getValues();
		...
		if($bGetProperty)
			$arResult["PROPERTIES2"] = $obElement->GetProperties();
		$arResult["DISPLAY_PROPERTIES"]=array();//$arParams["PROPERTY_CODE2"]
		foreach($arParams["PROPERTY_CODE2"] as $pid)
		{
			$prop = &$arResult["PROPERTIES2"][$pid];
			if(
				(is_array($prop["VALUE"]) && count($prop["VALUE"])>0)
				|| (!is_array($prop["VALUE"]) && strlen($prop["VALUE"])>0)
			)
			{
				$arResult["DISPLAY_PROPERTIES2"][$pid] = CIBlockFormatProperties::GetDisplayValue($arResult, $prop, "news_out");
			}
		}
